...
import org.artags.android.app.tv.util.HttpUtils;
    ...
    public static List<Tag> getTagList( String url )
    {
        ...
        try
        {
            ...
            for (int i = 0; i < max; i++)
            {
                ...
                tag.setThumbUrl(jsonTag.getString("imageUrl" ));
                tag.setDate(jsonTag.getString("date" ));
                tag.setLatitude(jsonTag.getString("lat" ));
                tag.setLongitude(jsonTag.getString("lon" ));
                list.add(tag);
            }
        }
